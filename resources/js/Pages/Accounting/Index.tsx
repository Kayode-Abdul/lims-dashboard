                    ...
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2 print:hidden">
                            <button
                                type="button"
                                onClick={() => {
                                    const totalBilled = sourcePatients.reduce((acc, curr) => acc + (curr.price - curr.discount), 0);
                                    const totalReceived = sourcePatients.reduce((acc, curr) => acc + Number(curr.period_payments || 0), 0);
                                    const totalOutstanding = sourcePatients.reduce((acc, curr) => acc + (curr.price - curr.discount - curr.amount_paid), 0);
                                    
                                    const message = `*Financial Summary for ${selectedSource?.source_name}*\n` +
                                                   `Period: ${formatDate(startDate)} to ${formatDate(endDate)}\n\n` +
                                                   `Total Billed: ${currency}${totalBilled.toLocaleString()}\n` +
                                                   `Received in Period: ${currency}${totalReceived.toLocaleString()}\n` +
                                                   `Total Outstanding: ${currency}${totalOutstanding.toLocaleString()}\n\n` +
                                                   `Generated by ${auth.user.lab?.name || 'Global Diagnostics'}`;
                                    
                                    window.open(`https://wa.me/?text=${encodeURIComponent(message)}`, '_blank');
                                }}
                                className="flex items-center gap-1 text-green-600 hover:text-green-700 border border-green-200 px-3 py-1.5 rounded text-sm font-bold bg-green-50 dark:bg-green-900/20 dark:border-green-900/50"
                            >
                            ...
                            <a
                                href={route('accounting.export-pdf', {
                                    source_type: selectedSource?.source_type,
                                    source_name: selectedSource?.source_name,
                                    start_date: startDate,
                                    end_date: endDate
                                })}
                                className="flex items-center gap-1 text-rose-600 hover:text-rose-700 border border-rose-200 px-3 py-1.5 rounded text-sm font-bold bg-rose-50 dark:bg-rose-900/20 dark:border-rose-900/50"
                            >
                            ...
                            <button
                                ...
                                className="flex items-center gap-1 text-indigo-600 hover:text-indigo-700 border border-indigo-200 px-3 py-1.5 rounded text-sm font-bold bg-indigo-50 dark:bg-indigo-900/20 dark:border-indigo-900/50"
                            >
